...
void lcdCreateCustomChar(unsigned int location, unsigned int *customArray) {
    lcd_send_command(0x40 | (location * 8)); //CGRAM address, for location only use 0-7
    for (unsigned int i = 0; i < 8; i++) {
        lcd_write_character(customArray[i]);
    }
}

unsigned int shipLCD[] = {
  0x04,
  0x0E,
  0x0E,
  0x0E,
